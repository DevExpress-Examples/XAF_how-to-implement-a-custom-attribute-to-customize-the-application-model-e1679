	...
	public class ViewsNodesGeneratorUpdater : ModelNodesGeneratorUpdater<ModelViewsNodesGenerator> {
		public override void UpdateNode(ModelNode node) {
			foreach(IModelView view in (IModelViews)node) {
				...
				foreach(DevExpress.ExpressApp.Model.IModelMember modelMember in view.ModelClass.AllMembers) {
				...
				if(!string.IsNullOrEmpty(propertiesToRemove)) {
					if(view is IModelDetailView) {
						foreach(string propertyName in propertiesToRemove.Split(new Char[] { ';' }, StringSplitOptions.RemoveEmptyEntries)) {
							IModelDetailViewItem viewItem = ((IModelDetailView)view).Items[propertyName];
							if(viewItem != null) {
								((IModelDetailView)view).Items.Remove(viewItem);
							}
						}
						IModelDetailViewLayout layoutModel = ((IModelDetailView)view).Layout;
						layoutModel.Remove(layoutModel[0]);
					...
					if(view is IModelListView) {
						foreach(string propertyName in propertiesToRemove.Split(new Char[] { ';' }, StringSplitOptions.RemoveEmptyEntries)) {
							...
							if(column != null) {
								((IModelListView)view).Columns.Remove(column);
							}
						}
					}
				}
